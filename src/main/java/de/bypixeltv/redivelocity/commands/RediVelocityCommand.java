...
public class RediVelocityCommand {
    ...
    private CommandAPICommand createProxyPlayersCommand() {
        return new CommandAPICommand("players")
                .withOptionalArguments(new StringArgument("proxy").replaceSuggestions(ArgumentSuggestions.stringCollection(args -> redisController.getAllHashFields("rv-proxies"))))
